<template>
  <div class="flex flex-col gap-6">
    ...
    <div v-if="$fetchState.pending">
      <cm-table
        :table-columns="tableOptions.columns"
        :table-data="[]"
      >
        <template #skeleton>
          <cm-table-skeleton
...
export default {
  ...
  data () {
    return {
       ...
       */
      rowsPerPageOptions: [{ label: '5' }, { label: '10' }, { label: '15' }, { label: '20' }]
    }
  },
  ...
  async fetch () {
    ...
    const { data, pagination } = await this.$axios.$get(this.apiEndpoint) || {}
    ...
    if (this.populateStore) { this.$store.commit(this.populateStore, dataValue) }
  },
  ...
  computed: {
     ...
     */
    tableColumns () {
      if (this.tableOptions_) { return this.tableOptions_.columns }
     ...
     */
    tableData () {
      if (this.tableOptions_) { return this.tableOptions_.data }
     ...
     */
    pagination () {
      if (this.tableOptions_) { return this.tableOptions_.pagination }
  ...
  mounted () {
     ...
     */
    this.$nuxt.$on("refresh:data_table", () => { this.$fetch() })
  },
  ...
  beforeDestory () {
     ...
     */
    this.$nuxt.$off("refresh:data_table", () => { this.$fetch() })
  }
}
</script>
